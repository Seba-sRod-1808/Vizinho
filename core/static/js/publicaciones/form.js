document.addEventListener('DOMContentLoaded', function() {
    // Buscar campos del formulario con guion bajo
    ...
    function updatePreview() {
      ...
      if (titulo || contenido) {
        previewContent.innerHTML = `
          ${titulo ? `<h5>${titulo}</h5>` : ''}
          ${contenido ? `<p>${contenido.replace(/\n/g, '<br>')}</p>` : '<p class="preview-empty"><i class="bi bi-file-text"></i> Escribe algo en el contenido...</p>'}
        `;
      } else {
        previewContent.innerHTML = `
          <div class="preview-empty">
            <i class="bi bi-file-text"></i>
            Comienza a escribir para ver una vista previa de tu publicación
          </div>
        `;
      }
    }
